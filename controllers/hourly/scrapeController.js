...
const scrapeAdditional = asyncHandler(async (req, res) => {
    request(urlToScrape, async (error, response, html) => {
        if (!error && response.statusCode === 200) {
            ...
            res.forEach(item => {
                item["Date"] = new Date(datum);
                item["Time"] = timeStriped;
                if (!item["Temperatura(°C)"] || item["Temperatura(°C)"] === "-" || item["Temperatura(°C)"] === "undefined") {
                    item["Temperatura(°C)"] = -100;
                }
                item["Temperatura(°C)"] = parseFloat(item["Temperatura(°C)"]);
                if (!item["Vlažnost(%)"] || item["Vlažnost(%)"] === "-" || item["Vlažnost(%)"] === "undefined") {
                    item["Vlažnost(%)"] = -100;
                }
                item["Vlažnost(%)"] = parseInt(item["Vlažnost(%)"]);
                if (!item["Pritisak(hPa)"] || item["Pritisak(hPa)"] === "-" || item["Pritisak(hPa)"] === "undefined") {
                    item["Pritisak(hPa)"] = -100;
                }
                item["Pritisak(hPa)"] = parseFloat(item["Pritisak(hPa)"]);
                if (!item["Vetarbrzina(m/s)"] || item["Vetarbrzina(m/s)"] === "-" || item["Vetarbrzina(m/s)"] === "undefined") {
                    item["Vetarbrzina(m/s)"] = -100;
                }
                if (item["Vetarbrzina(m/s)"] === "tiho") {
                    item["Vetarbrzina(m/s)"] = 0;
                }
                item["Vetarbrzina(m/s)"] = parseFloat(item["Vetarbrzina(m/s)"]);
                if (!item["Vetarpravac(°)"] || item["Vetarpravac(°)"] === "-" || item["Vetarpravac(°)"] === "undefined") {
                    item["Vetarpravac(°)"] = -100;
                }
    ...
    res.status(200).json({ message: `Podaci skinuta sa ${urlToScrape} i sačuvani u ${process.env.MONGODB_DB}!` });
});
